...
try:
    GBC=GradientBoostingClassifier(n_estimators=800,learning_rate=0.02,max_features=5)
    logging.info('Model Initiated successfully')
    GBC_f = GBC.fit(X_train, Y_train)
    logging.info('Model Fitted Successfully')
    Y_pred = GBC_f.predict(X_test)
    logging.info('Model Predicted Successfully')
    print('Confusion Matrix for RandomForestClassifier is :', confusion_matrix(Y_test,Y_pred))
    print('Accuracy for RandomForestClassifier is:- ', accuracy_score(Y_test, Y_pred))
    # Saving model to disk
    pickle.dump(GBC_f, open('model_gbc.pkl','wb'))
